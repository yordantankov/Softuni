    ...
    def animals_status(self):
        ...
        if "Lion" in animals.keys():
            animals_str += f"\n----- {len(animals['Lion'])} Lions:\n" + '\n'.join(
                str(animal) for animal in animals["Lion"])
        if "Tiger" in animals.keys():
            animals_str += f"\n----- {len(animals['Tiger'])} Tigers:\n" + '\n'.join(
                str(animal) for animal in animals["Tiger"])
        if "Cheetah" in animals.keys():
            animals_str += f"\n----- {len(animals['Cheetah'])} Cheetahs:\n" + '\n'.join(
                str(animal) for animal in animals["Cheetah"])
    ...
    def workers_status(self):
        ...
        if "Keeper" in workers.keys():
            workers_str += f"\n----- {len(workers['Keeper'])} Keepers:\n" + '\n'.join(
                str(worker) for worker in workers["Keeper"])
        if "Caretaker" in workers.keys():
            workers_str += f"\n----- {len(workers['Caretaker'])} Caretakers:\n" + '\n'.join(
                str(worker) for worker in workers["Caretaker"])
        if "Vet" in workers.keys():
            workers_str += f"\n----- {len(workers['Vet'])} Vets:\n" + '\n'.join(
                str(worker) for worker in workers["Vet"])
